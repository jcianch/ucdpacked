...
glance image-create --name centos6x64 --disk-format qcow2 --container-format bare --file /vagrant/osimages/CentOS-6-x86_64-GenericCloud.qcow2 --is-public True --progress --human-readable
...
glance image-create --name ubuntu1404x64 --disk-format qcow2 --container-format bare --file /vagrant/osimages/trusty-server-cloudimg-amd64-disk1.img --is-public True --progress --human-readable
imageid=`nova image-list | grep ubuntu1404x64 | awk '{ print $2}'`
nova image-meta $imageid set hw_qemu_guest_agent=yes

nova keypair-add admin_key > /vagrant/admin_key.priv
