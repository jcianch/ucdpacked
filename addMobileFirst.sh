...
curl -s -o /dev/null -w "%{http_code}" -u admin:${MY_UCD_PASSWORD}   --insecure -F "file=@$MEDIA_DIR/mobilefirst/MobileFirst.json;type=application/json;filename=MobileFirst.json" \
...
/opt/ibm-ucd/udclient/udclient -weburl http://${IPADDRESS}:${MY_UCD_HTTP_PORT} -authtoken $token addVersionFiles -component "IBM WAS Liberty" -version  "8.5.5.7" -base "$MEDIA_DIR/mobilefirst/artifacts/IBM WAS Liberty" 
